...
///Weather Status
    ...
    .then(data => {require('dotenv').config();
const Discord = require('discord.js')
const bot = new Discord.Client();
const commandHandler = "!";
const token = process.env.TOKEN;
...
if(!message.content.startsWith(commandHandler) || message.author.bot) return;
 ...
 const arguments = message.content.slice(commandHandler.length).split(/ +/);
...
///Weather Status
    ...
    const apiKey = "89ef8a05b6c964f4cab9e2f97f696c81"; 
    const kelvin = 273; 
    let location = capitalizeFirstLetter(arguments[0])
    let api = `https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=${apiKey}`;
    fetch(api)
    .then(resposne => resposne.json())
    .then(data => {
...
//Google Search
    ...
    let query = `https://www.google.com/search?q=${go}`
...
///Youtube Search
    ...
    let query = `https://www.youtube.com/results?search_query=${yt}`
...
ej: !rate btc o !rate eth`)
    ...
    let crypto = arguments[0].toUpperCase();
